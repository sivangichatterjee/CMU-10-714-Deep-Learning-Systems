...
def parse_mnist(image_filename, label_filename):
    ...
    with gzip.open(image_filename,'rb') as f:
      #Read header #> big endian #I= unsigned 4 byte integers
...
def softmax_loss(Z, y):
    ...
    correct_class_scores= Z[np.arange(N),y]
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0,n,batch):
      ...
      #2.gradient
      m=X_batch.shape[0]
      probs[np.arange(m),y_batch]-=1 #Z-Iy
      grad=(X_batch.T @ probs)/m
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0,n,batch):
      ...
      G2=probs
      G2[np.arange(m),y_b]-=1

      # grad W2
      grad_W2=(Z1_relu.T @ G2)/m
